...
export const sendBroadcast = async (req, res, next) => {
  try {
    ...
    res
      ...
      .json({
        message:
          "Broadcast started. Messages are being sent in the background.",
      });
      ...
      for (let i = 0; i < phones.length; i++) {
        try {
          let data = {
            ...
            mobile: `2${phones[i]}@s.whatsapp.net`,
            text: msg,
          };
          ...
          if (!whatsappMSG.ok) {
            ...
            console.error(
              `Failed to send to ${phones[i]}. Status: ${whatsappMSG.status}, Response: ${errorResponse}`
            );
          } else {
          ...
          console.error(
            `Error sending message to ${phones[i]}:`,
            error.message
          );
        }
    ...
    if (!res.headersSent) {
      res
        .status(500)
        .json({ message: "Failed to start broadcast. Check server logs." });
    }
  }
};
