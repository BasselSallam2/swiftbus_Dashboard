...
function isBeforeDateTimeWithOffset(dateStr, timeStr, numberOfHours) {
	...
	const filteredTrips = await Promise.all(
		trips.map(async (trip) => {
			try {
				...
				// const isValid = isBeforeDateTimeWithOffset(formattedGodate , ArriveTest?.arrivaleTime ,0) ;
