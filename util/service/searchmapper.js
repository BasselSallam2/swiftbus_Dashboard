    ...
    searchresultGo.map(async (trip) => {
      try {
        ...
        const StationfromDetails = trip.StationDetails.find(
          (stationDetail) => stationDetail.station.name === from
        );

        const StationtoDetails = trip.StationDetails.find(
          (stationDetail) => stationDetail.station.name === to
        );
        ...
        const fare = trip.cost.find(
          (t) => t.fromCityId === cityfrom.city.id && t.toCityId === cityto.city.id
        );
        ...
        const [month, day, year] = godate.split("/");
         ...
         const ResultReservation = trip.Reservations.filter((reserve) => reserve.trip_date == formattedGodate ) ;
